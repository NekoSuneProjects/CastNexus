...
function profileMusicState(account, profile) {
  ...
  if (bucket) return { profileId: key, tracks: Array.isArray(bucket.tracks) ? bucket.tracks : [], settings: bucket.settings || {} };
...
class Music24Worker {
  ...
  getNow() {
    ...
    const now = { ...this.now };
    if (now.mode === "playing" && now.track) now.positionS = Math.max(0, Number(now.positionS || 0) + (Date.now() - this.nowFetchedAt) / 1000);
    return now;
  }
  musicFilePathFor(trackId) {
    const musicState = profileMusicState(this.account, this.profile);
    const track = musicState?.tracks.find(t => t.id === trackId);
    if (!track) return null;
    return path.join(MUSIC_DIR, sanitizeSegment(this.accountId), musicState.profileId, track.filename);
  }
  async pollNow() {
    try {
...
async function shutdown(signal) {
  ...
  for (const worker of workers.values()) { try { await worker.stop(); } catch {} }
  process.exit(0);
}
