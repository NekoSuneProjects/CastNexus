...
const { liveInputArgs, liveMuxArgs } = require("./rtmp-pipeline");
...
function installPublicRepublishSpawnPolicy({ logger = console } = {}) {
  ...
  childProcess.spawn = function castNexusSpawn(command, args, options) {
    ...
    logger.log?.(`[dashboard] stable public republish pipeline -> ${dest.replace(/^rtmp:\/\/127\.0\.0\.1:1935\//, "")}`);
    ...
    if (child?.stderr) {
      child.stderr.on("data", chunk => {
        ...
        if (!line) return;
        if (process.env.PUBLIC_REPUBLISH_DEBUG === "true" || /error|failed|invalid|refused|non-monoton|queue|timestamp/i.test(line)) {
