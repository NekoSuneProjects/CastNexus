function renderSettings() {
  ...
  return `
    ${pageHead("SYSTEM", "Settings", "Connection details and safe defaults for this RestreamNode instance.")}
...
function renderPage() {
  ...
  if (!root) return;
  $("#page-title").textContent = PAGE_TITLES[S.page] || "RestreamNode";
...
function openDestinationModal(dest = null) {
  modalShell(dest ? "Edit destination" : "Add destination", "OUTPUT ROUTE", `<div class="form-grid"><div class="full"><label>Name</label><input id="dest-name" value="${esc(dest?.name || "")}" placeholder="YouTube Live"></div><div class="full"><label>${dest ? "New URL (leave blank to keep current)" : "RTMP / RTMPS / SRT URL"}</label><input id="dest-url" placeholder="rtmps://…" value=""></div></div><div id="modal-error" class="form-error"></div>`, `<button class="btn btn-ghost" data-modal-close>Cancel</button><button class="btn btn-primary" id="dest-save">Save destination</button>`);
  $$('[data-modal-close]').forEach(b => b.onclick = closeModal);
  $("#dest-save").onclick = async () => {
    const name = $("#dest-name").value.trim(), url = $("#dest-url").value.trim();
    try {
      if (dest) {
        const body = { name }; if (url) body.url = url;
        await api(`/api/destinations/${encodeURIComponent(dest.id)}`, { method:"PUT", body });
      } else {
        await api("/api/destinations", { method:"POST", body:{ name, url } });
      }
...
function openProfileModal(mode, profile = null) {
  ...
  modalShell(editing ? "Edit profile" : `New ${MODE_LABELS[chosenMode]} profile`, "PROFILE", `<div class="form-grid"><div class="full"><label>Name</label><input id="pf-name" value="${esc(profile?.name || (chosenMode === "pc" ? "PC Gaming" : chosenMode === "console" ? "Console Gaming" : "24/7 Music"))}"></div><div><label>Mode</label><select id="pf-mode" ${editing ? "disabled" : ""}><option value="pc" ${chosenMode==="pc"?"selected":""}>PC Streaming</option><option value="console" ${chosenMode==="console"?"selected":""}>Console Streaming</option><option value="music" ${chosenMode==="music"?"selected":""}>24/7 Music</option></select></div><div><label>Profile colour</label><input id="pf-color" type="color" value="${esc(profile?.color || PROFILE_COLORS[chosenMode])}"></div></div><div id="modal-error" class="form-error"></div>`, `<button class="btn btn-ghost" data-modal-close>Cancel</button><button class="btn btn-primary" id="pf-save">Save profile</button>`);
  $$('[data-modal-close]').forEach(b=>b.onclick=closeModal);
  $("#pf-save").onclick = async () => {
    const name=$("#pf-name").value.trim(), m=editing ? profile.mode : $("#pf-mode").value, color=$("#pf-color").value;
    if(!name) return $("#modal-error").textContent="Name is required";
    if(editing){ profile.name=name; profile.color=color; }
    else { const p=createProfile(m,name); p.color=color; S.profiles.push(p); }
...
function openTrackModal(track) {
  modalShell("Edit track metadata", "MUSIC LIBRARY", `<div class="form-grid"><div class="full"><label>Title</label><input id="track-title" value="${esc(track.title || "")}"></div><div class="full"><label>Artist</label><input id="track-artist" value="${esc(track.artist || "")}"></div></div><div id="modal-error" class="form-error"></div>`, `<button class="btn btn-ghost" data-modal-close>Cancel</button><button class="btn btn-primary" id="track-save">Save metadata</button>`);
  $$('[data-modal-close]').forEach(b=>b.onclick=closeModal);
  $("#track-save").onclick=async()=>{ try{ await api(`/api/music/tracks/${encodeURIComponent(track.id)}`,{method:"PUT",body:{title:$("#track-title").value.trim(),artist:$("#track-artist").value.trim()}}); closeModal(); await refreshAndRender(); toast("Track updated","success"); }catch(e){ $("#modal-error").textContent=e.message; } };
}
