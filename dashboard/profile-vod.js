...
function normalizeRemoteUrl(kind, raw) {
  ...
  if (kind === "twitch-live" && /^[A-Za-z0-9_]{2,32}$/.test(value)) {
...
function ytDlpBase(kind) {
  ...
  if (kind === "youtube") {
    args.push("--js-runtimes", "deno", "--remote-components", "ejs:npm");
  }
  return args;
}
...
function ffmpegArgsFor(source, outputUrl) {
  ...
  // Twitch HLS/VOD is already Twitch-compatible H.264/AAC, so keep it cheap.
  if (source.kind === "twitch-live" || source.kind === "twitch-vod") {
    ...
    args.push(
      ...
      "-r", String(Number(process.env.VOD_FPS || 30)),
      "-c:v", process.env.VOD_VIDEO_ENCODER || "libx264",
      "-preset", process.env.VOD_X264_PRESET || "veryfast",
      "-tune", "zerolatency",
      "-pix_fmt", "yuv420p",
      "-profile:v", "high",
      "-level:v", "4.2",
      ...
      "-keyint_min", "60",
      "-sc_threshold", "0",
      "-b:v", process.env.VOD_VIDEO_BITRATE || "6000k",
...
function createProfileVodService({ state, saveState, vodDir, maxBytes, probeDurationSeconds, rtmpOrigin = "rtmp://127.0.0.1:1935" }) {
  const sessions = new Map(); // accountId -> running relay/rerun
  ...
  async function sourceForItem(account, info, item) {
    ...
    if (item.kind === "youtube" || item.kind === "twitch-vod") {
      return { kind: item.kind, inputs: await resolveRemote(item.kind, item.url) };
    }
  ...
  async function startResolved(account, info, source, meta) {
    ...
    const launch = () => {
      ...
      const child = spawn(FFMPEG_BIN, ffmpegArgsFor(source, outputUrl), { stdio: ["ignore", "ignore", "pipe"] });
      session.child = child;
      let stderrTail = "";
      child.stderr.on("data", chunk => {
  ...
  async function startTwitchLive(account, profileId, channelOrUrl) {
    ...
    const channel = new URL(url).pathname.split("/").filter(Boolean)[0] || "Twitch Live";
  ...
  function createApiRouter() {
    ...
    router.post("/remote", async (req, res) => {
      ...
      try {
        ...
        const item = {
          ...
          url: metadata.webpageUrl || normalized,
    ...
    router.post("/play", async (req, res) => {
      ...
      try {
        const status = await playItem(req.account, profileId, itemId, Boolean(loop));
        res.json({ ok: true, status });
      } catch (err) {
        res.status(err.code === "YOUTUBE_CHALLENGE" ? 422 : 400).json({ error: err.message, code: err.code || "PLAY_FAILED" });
      }
    });
    ...
    router.post("/twitch-live", async (req, res) => {
      ...
      try {
        const status = await startTwitchLive(req.account, profileId, channel);
        res.json({ ok: true, status });
      } catch (err) {
        res.status(400).json({ error: err.message, code: err.code || "TWITCH_LIVE_FAILED" });
      }
    });
  ...
  return { createApiRouter, bucketFor, diskDir, publicStatus, stop, playItem, startTwitchLive, normalizeRemoteUrl, resolveRemote, inspectRemote, ffmpegArgsFor };
}
