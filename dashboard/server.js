...
function loadState() {
  if (fs.existsSync(STATE_FILE)) {
    ...
    if (loaded.destinations && !loaded.accounts) {
      return { accounts:{}, pendingLegacyDestinations:loaded.destinations, sessionSecret:crypto.randomBytes(32).toString("hex") };
    }
...
function getAccountByLogin(login) { return Object.values(state.accounts).find(a => a.twitchLogin === login); }
...
const profileVod = createProfileVodService({
  state, saveState, vodDir:VOD_DIR, maxBytes:VOD_MAX_BYTES,
...
function activeSourceFor(accountId) { const activePath = activeFeedPath.get(accountId); return activePath ? liveSessions.get(activePath)?.source ?? null : null; }
...
function enterGrace(account, pathName, source) {
  ...
  const timer = setTimeout(() => { console.log(`[dashboard] ${account.twitchLogin} did not reconnect within the grace window - ending stream`); graceState.delete(account.twitchUserId); stopOutputsFor(account.twitchUserId); }, RECONNECT_GRACE_MS);
...
async function pollLive() {
  ...
  for (const [pathName, live] of [...liveSessions.entries()]) {
    ...
    if (account && live.source === "rerun") {
      ...
      if (!["starting","playing","restarting"].includes(rerunState)) {
        clearGrace(live.accountId); stopOutputsFor(live.accountId);
  ...
  for (const pathName of readyPaths) {
    ...
    if (wasInGrace) { clearGrace(account.twitchUserId); console.log(`[dashboard] ${account.twitchLogin} reconnected within the grace window`); }
...
app.get("/auth/twitch", (req,res) => {
  ...
  url.searchParams.set("client_id",TWITCH_CLIENT_ID); url.searchParams.set("redirect_uri",TWITCH_REDIRECT_URI); url.searchParams.set("response_type","code"); url.searchParams.set("scope",""); url.searchParams.set("state",oauthState);
...
app.get("/auth/twitch/callback", async (req,res) => {
  ...
  try {
    ...
    const userRes = await fetch("https://api.twitch.tv/helix/users", { headers:{ "Authorization":`Bearer ${tokenData.access_token}`, "Client-Id":TWITCH_CLIENT_ID } });
    const userData = await userRes.json(); if (!userRes.ok || !userData.data?.[0]) throw new Error("failed to fetch Twitch user info");
    const twitchUser = userData.data[0];
    let account = state.accounts[twitchUser.id]; const isNewAccount = !account;
    if (!account) {
      account = {
        twitchUserId:twitchUser.id, streamKey:null, pcKey:generatePcKey(), sourceMode:null,
        destinations:state.pendingLegacyDestinations || [], overlayConfig:defaultOverlayConfig(), overlays:[],
        musicTracks:[], musicSettings:defaultMusicSettings(), musicProfiles:{}, vodProfiles:{},
        currentScene:null, compositorEnabled:false, recordingEnabled:false, youtubeUploadHistory:[], createdAt:new Date().toISOString(),
      };
      state.accounts[twitchUser.id] = account;
      if (state.pendingLegacyDestinations) { console.log(`[dashboard] migrated legacy destinations to new account ${twitchUser.login}`); state.pendingLegacyDestinations = null; }
    }
    account.twitchLogin = twitchUser.login; account.displayName = twitchUser.display_name; account.profileImageUrl = twitchUser.profile_image_url; saveState(state);
    req.session.accountId = twitchUser.id;
    recordings.ensureConfig(account).catch(err => console.warn(`[recordings] path config: ${err.message}`));
    profileVod.refreshTwitchCatalog(account).catch(err => console.warn(`[twitch-vods] initial refresh: ${err.message}`));
...
app.get("/auth/youtube", requireAuth, (req,res) => {
  if (!youtubeUploads.configured()) return res.status(500).send("YouTube OAuth is not configured");
  const oauthState = crypto.randomBytes(24).toString("hex"); req.session.youtubeOauthState = oauthState;
  const url = new URL("https://accounts.google.com/o/oauth2/v2/auth");
  url.searchParams.set("client_id",YOUTUBE_CLIENT_ID); url.searchParams.set("redirect_uri",YOUTUBE_REDIRECT_URI); url.searchParams.set("response_type","code");
  url.searchParams.set("scope","https://www.googleapis.com/auth/youtube.upload"); url.searchParams.set("access_type","offline"); url.searchParams.set("prompt","consent"); url.searchParams.set("include_granted_scopes","true"); url.searchParams.set("state",oauthState);
  res.redirect(url.toString());
});
app.get("/auth/youtube/callback", async (req,res) => {
  const account = getAccount(req.session.accountId);
  if (!account) return res.status(401).send("Sign in to CastNexus with Twitch first");
  const { code, state:returnedState, error, error_description } = req.query;
  if (error) return res.status(400).send(`YouTube authorization failed: ${error_description || error}`);
  if (!code || !returnedState || returnedState !== req.session.youtubeOauthState) return res.status(400).send("Invalid YouTube OAuth state");
  delete req.session.youtubeOauthState;
  try { youtubeUploads.storeTokens(account, await youtubeUploads.exchangeCode(String(code))); res.redirect("/?youtube=connected"); }
  catch (err) { console.error("[youtube] OAuth error:",err.message); res.status(500).send(`YouTube authorization failed: ${err.message}`); }
});
...
app.post("/api/pckey/regenerate", requireAuth, (req,res) => { req.account.pcKey = generatePcKey(); saveState(state); res.json({ ok:true, pcKey:req.account.pcKey }); });
...
app.get("/api/recordings/play", requireAuth, async (req,res) => {
  try {
    const library = await recordings.list(req.account);
    const segment = library.segments.find(s => s.start === req.query.start);
    if (!segment) return res.status(404).send("recording not found");
    const upstream = await fetch(recordings.playbackUrl(req.account, segment.start, segment.duration, "mp4"));
    if (!upstream.ok || !upstream.body) return res.status(upstream.status).send("recording playback unavailable");
...
app.post("/api/youtube/upload-recording", requireAuth, async (req,res) => {
  try {
    youtubeUploads.assertQuota(req.account);
    if (!youtubeUploads.status(req.account).connected) return res.status(409).json({ error:"Connect YouTube before uploading a MediaMTX recording" });
    const library = await recordings.list(req.account);
    const segment = library.segments.find(s => s.start === req.body?.start);
    if (!segment) return res.status(404).json({ error:"recording not found" });
    const jobId = crypto.randomUUID();
    const job = { id:jobId, accountId:req.account.twitchUserId, state:"queued", start:segment.start, title:String(req.body?.title || `CastNexus recording ${segment.start}`), createdAt:new Date().toISOString(), error:null, result:null };
    youtubeUploadJobs.set(jobId, job);
    res.status(202).json({ ok:true, jobId });
    setImmediate(async () => {
      try {
        job.state = "uploading";
        const result = await youtubeUploads.uploadRecording(req.account, segment, {
          title:job.title,
          description:String(req.body?.description || ""),
          privacyStatus:String(req.body?.privacyStatus || "private"),
          categoryId:String(req.body?.categoryId || "20"),
          tags:Array.isArray(req.body?.tags) ? req.body.tags : String(req.body?.tags || "").split(",").map(x => x.trim()).filter(Boolean),
        });
        job.state = "completed"; job.result = { id:result.id, url:result.url, privacyStatus:result.privacyStatus }; job.completedAt = new Date().toISOString();
        req.account.youtubeUploadHistory.push({ jobId, start:segment.start, title:job.title, ...job.result, completedAt:job.completedAt });
        req.account.youtubeUploadHistory = req.account.youtubeUploadHistory.slice(-100); saveState(state);
      } catch (err) { job.state = "error"; job.error = err.message; job.completedAt = new Date().toISOString(); }
    });
  } catch (err) { res.status(err.code === "YOUTUBE_NOT_WHITELISTED" ? 403 : err.code === "YOUTUBE_SOFT_QUOTA" ? 429 : 400).json({ error:err.message, code:err.code || "YOUTUBE_UPLOAD_FAILED" }); }
});
...
function sourcesStatusFor(account){const mode=account.sourceMode||"both",pcEnabled=mode==="pc"||mode==="both";const sources={console:{enabled:mode==="console"||mode==="both",live:false},pc:{enabled:pcEnabled,live:false},rerun:{enabled:pcEnabled,live:false}};for(const live of liveSessions.values())if(live.accountId===account.twitchUserId&&sources[live.source])sources[live.source].live=true;return{sources,activeSource:activeSourceFor(account.twitchUserId)};}
app.get("/api/status", requireAuth, (req,res)=>{const account=req.account,{sources,activeSource}=sourcesStatusFor(account);res.json({twitchLogin:account.twitchLogin,displayName:account.displayName,profileImageUrl:account.profileImageUrl,needsSourceMode:!account.sourceMode,needsStreamKey:needsStreamKeyFor(account),streamKeyMasked:account.streamKey?maskSecret(account.streamKey):null,sourceMode:account.sourceMode||null,live:!!activeSource,sources,activeSource,rerun:profileVod.publicStatus(account.twitchUserId),recordingEnabled:!!account.recordingEnabled,encoder:gpuEncoder.status(),graceUntil:graceState.get(account.twitchUserId)?.deadline??null,pcServer:`rtmp://${MEDIA_HOST}:1935/${PC_APP}`,pcKey:account.pcKey,mediaHost:MEDIA_HOST,playback:playbackUrlsFor(req,account),destinations:account.destinations.map(d=>({id:d.id,name:d.name,urlMasked:maskUrl(d.url),layout:normaliseLayout(d.layout),enabled:d.enabled,active:activeDestinations.has(`${account.twitchUserId}:${d.id}`)}))});});

app.post("/api/destinations", requireAuth, requireOnboarded, (req,res)=>{const{name,url,layout}=req.body||{};if(!name||!String(name).trim())return res.status(400).json({error:"name is required"});if(!url||!DESTINATION_URL_RE.test(url))return res.status(400).json({error:DESTINATION_URL_HINT});if(layout!==undefined&&!OUTPUT_LAYOUTS.includes(layout))return res.status(400).json({error:`layout must be one of ${OUTPUT_LAYOUTS.join(", ")}`});const dest={id:crypto.randomUUID(),name:String(name).trim(),url:String(url).trim(),layout:normaliseLayout(layout),enabled:false};req.account.destinations.push(dest);saveState(state);res.json({ok:true,id:dest.id});});
...
app.listen(PORT,()=>{
  const encoder = gpuEncoder.status().selected;
  console.log(`[dashboard] listening on :${PORT}`);
  console.log(`[dashboard] video encoder: ${encoder.label}${encoder.hardware ? " (hardware)" : " (software fallback)"}`);
  if(!TWITCH_CLIENT_ID||!TWITCH_CLIENT_SECRET)console.warn("[dashboard] TWITCH_CLIENT_ID/TWITCH_CLIENT_SECRET are not set - Twitch login/API discovery will not work");
  if(!YOUTUBE_CLIENT_ID||!YOUTUBE_CLIENT_SECRET)console.warn("[dashboard] YOUTUBE_CLIENT_ID/YOUTUBE_CLIENT_SECRET are not set - YouTube recording uploads will be disabled");
});
