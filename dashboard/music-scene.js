  ...
  return `
    ...
    <script>
      (function(){
        ...
        function ensureGraph(){if(audioContext){if(audioContext.state==="suspended")audioContext.resume().catch(function(){});return}try{var AC=window.AudioContext||window.webkitAudioContext;if(!AC)return;audioContext=new AC();analyser=audioContext.createAnalyser();analyser.fftSize=256;analyser.smoothingTimeConstant=.72;analyser.minDecibels=-88;analyser.maxDecibels=-18;gain=audioContext.createGain();source=audioContext.createMediaElementSource(audio);source.connect(analyser);analyser.connect(gain);gain.connect(audioContext.destination);if(audioContext.state==="suspended")audioContext.resume().catch(function(){})}catch(e){analyser=null}}
        ...
        function draw(){if(!canvas)return;var ctx=canvas.getContext("2d");if(!ctx)return;ensureGraph();var dpr=Math.min(2,window.devicePixelRatio||1),w=Math.max(1,Math.floor(canvas.clientWidth*dpr)),h=Math.max(1,Math.floor(canvas.clientHeight*dpr));if(canvas.width!==w)canvas.width=w;if(canvas.height!==h)canvas.height=h;ctx.clearRect(0,0,w,h);var target;if(analyser&&audioContext&&audioContext.state==="running"&&currentNow&&currentNow.mode==="playing"){var data=new Uint8Array(analyser.frequencyBinCount);analyser.getByteFrequencyData(data);target=analyseBars(data,BAR_COUNT)}else target=procedural(BAR_COUNT);drawBars(ctx,smoothBars(target),w,h,dpr)}
