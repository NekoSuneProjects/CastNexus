...
function musicSceneFragment(login, cfg = {}, query = {}, account, profileId = null) {
  ...
  return `
    <style>
      ...
      .rn-progress>span{display:block;width:0%;height:100%;border-radius:inherit;background:linear-gradient(90deg,var(--rn-accent-solid),#8a2bff,#ff2bd6,var(--rn-accent-solid));background-size:220% 100%;box-shadow:0 0 16px var(--rn-accent-glow);will-change:width,background-position}
      ...
      #rn-spectrum{width:100%;height:100%;display:block;filter:none}
    ...
    <script>
      (function(){
        ...
        function renderTimeline(){var playing=currentNow&&currentNow.mode==="playing"&&currentNow.track,pos=playing?Number(currentNow.positionS||0)+(performance.now()-currentNowAt)/1000:0,dur=playing?Number(currentNow.durationS||0):0;pos=Math.max(0,dur>0?Math.min(dur,pos):pos);var bar=document.getElementById("rn-progress-bar");if(bar){bar.style.width=dur>0?Math.max(0,Math.min(100,pos/dur*100))+"%":"0%";bar.style.backgroundPosition=((performance.now()/18)%220)+"% 0"}var label=fmt(pos)+" / "+fmt(dur);if(label!==lastTimeLabel){lastTimeLabel=label;var time=document.getElementById("rn-track-time");if(time)time.textContent=label}}
