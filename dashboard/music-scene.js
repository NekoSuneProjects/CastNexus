...
function musicSceneFragment(login, cfg = {}, query = {}, account, profileId = null) {
  ...
  return `
    <style>
      ...
      .rn-art-column{min-width:0;display:grid;place-items:center}.rn-cover-wrap{position:relative;width:min(34vw,48vh,500px);min-width:0;aspect-ratio:1}
      ...
      .rn-cover{position:absolute;inset:0;z-index:3;width:100%;height:100%;object-fit:cover;border-radius:clamp(12px,1.2vw,24px);border:1px solid var(--rn-accent-line);box-shadow:0 24px 80px rgba(0,0,0,.48),0 0 50px var(--rn-accent),inset 0 1px rgba(255,255,255,.08);background:linear-gradient(135deg,var(--rn-accent),rgba(138,43,255,.18))}
      .rn-cover-placeholder{position:absolute;inset:0;z-index:2;border-radius:clamp(12px,1.2vw,24px);border:1px solid var(--rn-accent-line);display:flex;align-items:center;justify-content:center;font-size:clamp(48px,8vw,120px);color:var(--rn-accent-solid);background:radial-gradient(circle at 42% 36%,var(--rn-accent),rgba(5,6,10,.96) 66%);text-shadow:0 0 24px var(--rn-accent-glow)}
      .rn-vinyl{position:absolute;z-index:1;top:50%;right:-29%;width:78%;aspect-ratio:1;border-radius:50%;transform:translateY(-50%);opacity:.78;border:1px solid var(--rn-accent-line);box-shadow:0 18px 58px rgba(0,0,0,.62),0 0 34px rgba(0,240,255,.06);background:radial-gradient(circle at 50% 50%,var(--rn-accent-solid) 0 4%,#0a0d14 4.5% 13%,#171c29 13.5% 16%,#070910 16.5% 22%,#151a25 22.5% 32%,#070910 32.5% 34%,#151a25 34.5% 46%,#070910 46.5% 48%,#111621 48.5% 64%,#06080d 64.5% 66%,#11151e 66.5%);animation:rn-vinyl-spin 16s linear infinite}
      ...
      .rn-music-title{max-width:100%;font-size:clamp(22px,4.2vw,78px);font-weight:850;line-height:1.02;letter-spacing:-.035em;text-shadow:0 0 22px var(--rn-accent-glow);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
      .rn-music-artist{max-width:100%;font-size:clamp(12px,1.55vw,28px);color:rgba(230,247,255,.72);letter-spacing:.035em;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
      ...
      .rn-layout-vertical .rn-vinyl{right:-22%;width:70%}
    ...
    <main class="rn-music-stage rn-layout-${layout}${audioOnly ? " rn-audio-only" : ""}" style="${sceneStyleVars(accent)}">
      ...
      <div class="rn-music-main">
        <div class="rn-art-column"><div class="rn-cover-wrap"><div class="rn-cover-halo"></div><div class="rn-vinyl"></div><div id="rn-cover-placeholder" class="rn-cover-placeholder">&#9835;</div>${coverUrl ? `<img id="rn-cover" class="rn-cover" src="${escapeHtml(coverUrl)}" alt="" onerror="this.style.display='none'">` : ""}</div></div>
    ...
    <script>
      (function(){
        ...
        var analyser=null,audioContext=null,source=null,gain=null,loadedTrackId=null,currentNow=null;
        ...
        function ensureGraph(){if(audioContext)return;try{var AC=window.AudioContext||window.webkitAudioContext;if(!AC)return;audioContext=new AC();analyser=audioContext.createAnalyser();analyser.fftSize=256;analyser.smoothingTimeConstant=.82;analyser.minDecibels=-88;analyser.maxDecibels=-18;gain=audioContext.createGain();source=audioContext.createMediaElementSource(audio);source.connect(analyser);analyser.connect(gain);gain.connect(audioContext.destination);if(audioContext.state==="suspended")audioContext.resume().catch(function(){})}catch(e){analyser=null}}
        function apply(now){currentNow=now;var playing=now&&now.mode==="playing"&&now.track;var mode=document.getElementById("rn-mode"),title=document.getElementById("rn-track-title"),artist=document.getElementById("rn-track-artist"),time=document.getElementById("rn-track-time"),bar=document.getElementById("rn-progress-bar");if(mode)mode.textContent=playing?"ON AIR · NOW PLAYING":"ON AIR · STANDBY";if(title)title.textContent=playing?(now.track.title||"Untitled"):${JSON.stringify(fallbackTitle)};if(artist)artist.textContent=playing?(now.track.artist||"—"):"Waiting for the next track";var pos=playing?Number(now.positionS||0):0,dur=playing?Number(now.durationS||0):0;if(time)time.textContent=fmt(pos)+" / "+fmt(dur);if(bar)bar.style.width=dur>0?Math.max(0,Math.min(100,pos/dur*100))+"%":"0%";if(!playing){if(!audio.paused)audio.pause();return}ensureGraph();var vol=Math.max(0,Math.min(1,Number(now.volume == null ? .7 : now.volume)));if(gain)gain.gain.value=vol;else audio.volume=vol;if(now.track.id!==loadedTrackId){loadedTrackId=now.track.id;audio.src=PUBLIC_BASE+"/file/"+encodeURIComponent(now.track.id);var wanted=Math.max(0,Number(now.positionS||0));var seek=function(){try{audio.currentTime=Number.isFinite(audio.duration)&&audio.duration>0?Math.min(wanted,Math.max(0,audio.duration-.05)):wanted}catch(e){}audio.play().catch(function(){});audio.removeEventListener("loadedmetadata",seek)};audio.addEventListener("loadedmetadata",seek);audio.play().catch(function(){})}else if(Math.abs((audio.currentTime||0)-pos)>2.5){try{audio.currentTime=pos}catch(e){}}}
        ...
        function procedural(n){var t=performance.now()/680,bars=[];for(var i=0;i<n;i++){var env=.34+.12*Math.sin(i*.19+.8),wave=.09*Math.sin(t*1.55+i*.39)+.055*Math.sin(t*.82-i*.21);bars.push(Math.max(.055,Math.min(.55,env+wave)))}return bars}
        ...
        function draw(){if(!canvas)return;var ctx=canvas.getContext("2d");if(!ctx)return;var dpr=Math.min(2,window.devicePixelRatio||1),w=Math.max(1,Math.floor(canvas.clientWidth*dpr)),h=Math.max(1,Math.floor(canvas.clientHeight*dpr));if(canvas.width!==w)canvas.width=w;if(canvas.height!==h)canvas.height=h;ctx.clearRect(0,0,w,h);var target;if(analyser&&currentNow&&currentNow.mode==="playing"){var data=new Uint8Array(analyser.frequencyBinCount);analyser.getByteFrequencyData(data);target=analyseBars(data,BAR_COUNT)}else target=procedural(BAR_COUNT);drawBars(ctx,smoothBars(target),w,h,dpr)}
        ...
        poll();clock();window.__rnMusicPoll&&clearInterval(window.__rnMusicPoll);window.__rnMusicPoll=setInterval(poll,POLL_MS);window.__rnMusicDraw&&clearInterval(window.__rnMusicDraw);window.__rnMusicDraw=setInterval(draw,1000/RENDER_FPS);window.__rnMusicClock&&clearInterval(window.__rnMusicClock);window.__rnMusicClock=setInterval(clock,1000);draw();
      })();
    </script>`;
}
