...
// CastNexus executable. Put that directory first so the dashboard resolves
// those known binaries before anything installed globally. FFmpeg remains a
// host prerequisite and is also where CastNexus detects NVENC/QSV/AMF/etc.
...
function nativeUpdateNotice(release) {
  const tag=release.tag_name||release.name||"new release";const text=`${tag} is available. Open CastNexus to view the release and update instructions.`;
...
if(!fs.existsSync(mediamtxBin)){console.error(`[CastNexus] MediaMTX binary not found at ${mediamtxBin}`);console.error("[CastNexus] place mediamtx next to this executable or set MEDIAMTX_BIN.");process.exit(1);}
...
mtx.on("exit",code=>{console.error(`[CastNexus] MediaMTX exited (code ${code}), shutting down`);process.exit(1);});

setTimeout(()=>{
  // Static require is intentional: @yao-pkg/pkg traces the dashboard modules.
  require("../dashboard/server.js");
  const url=`http://localhost:${process.env.DASHBOARD_PORT}`;
  console.log(`[CastNexus] Studio ready at ${url}`);
  console.log("[CastNexus] desktop mode includes GPU encoder auto-detection, MediaMTX recordings, Twitch VOD discovery and yt-dlp/Deno VOD resolving. Console DNS/intercept remains Docker/Linux-specific.");
