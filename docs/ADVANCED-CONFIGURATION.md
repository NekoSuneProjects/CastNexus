...
This uses the CastNexus oauth-broker service so the installation never needs provider client secrets. `CASTNEXUS_OAUTH_BROKER_URL` is required and must be an `https://` URL - there is no local/BYO-credential mode.
...
YouTube sign-in goes through the oauth-broker service, the same as Twitch - there are no local Google client credentials to configure.
