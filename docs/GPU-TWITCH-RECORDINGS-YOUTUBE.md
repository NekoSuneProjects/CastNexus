...
CastNexus can upload a recorded program directly to YouTube using YouTube Data API v3, authorised through the oauth-broker service - there is no separate Google Cloud setup to do on the dashboard side. See INSTALL.md's "Self-hosting your own oauth-broker" section if you want to use your own Google OAuth application instead of the official broker.
